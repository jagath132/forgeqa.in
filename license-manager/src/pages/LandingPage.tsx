...
const TESTIMONIALS = [
  {
    ...
    avatar: 'RS',
    quote:
      'ForgeKey transformed how we manage software licenses. The verification workflow alone saved us dozens of hours every month.',
  },
  {
    name: 'Priya Patel',
    ...
    avatar: 'PP',
    quote:
      'We evaluated over a dozen licensing platforms. ForgeKey was the only one that checked every box — and their API is exceptional.',
  },
  {
    name: 'Arjun Mehta',
    ...
    avatar: 'AM',
    quote:
      'Setting up licensing infrastructure used to take weeks. With ForgeKey, we were live and issuing keys in an afternoon.',
  },
  {
    name: 'Neha Gupta',
    ...
    avatar: 'NG',
    quote:
      "Batch key generation and customer management are top-notch. It's rare to find a tool this polished in this space.",
  },
  {
    name: 'Vikram Singh',
    role: 'CEO, CodeForge',
    avatar: 'VS',
    quote:
      'ForgeKey gives us the confidence to scale our licensing operation without worrying about leaks or mismanagement.',
  },
];
...
function ScrollReveal({
  ...
  return (
    <div
      ref={ref}
      className={`${visible ? 'fk-visible' : ''} ${className}`}
  ...
  return (
    <div>
      <div className="fk-showcase-window-bar">
        ...
        <span className="fk-showcase-dot" />
        <span className="fk-showcase-dot" />
        <span style={{ fontSize: 11, color: 'var(--fk-text-muted)', marginLeft: 8 }}>
          dashboard
        </span>
      </div>
      <div style={{ padding: 20 }}>
        ...
        <div className="fk-showcase-mock-card">
          ...
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="fk-showcase-mock-row"
              style={{
                padding: '8px 0',
                borderBottom: i < 3 ? '1px solid var(--fk-border)' : 'none',
              }}
            >
              <div className="fk-showcase-mock-bar short" />
              <div className="fk-showcase-mock-bar" style={{ flex: 0.6 }} />
              <div
                className="fk-showcase-mock-bar"
                style={{ flex: 0.3, background: 'var(--fk-accent-subtle)' }}
              />
            </div>
          ))}
        </div>
...
function SlideKeys() {
  return (
    <div>
      ...
      <div style={{ padding: 20 }}>
        <div className="fk-showcase-mock-label" style={{ marginBottom: 10 }}>
          Product Keys
        </div>
          ...
          {['FRGK-A1B2-C3D4', 'FRGK-E5F6-G7H8', 'FRGK-I9J0-K1L2'].map((key, i) => (
            <div
              ...
              style={{
                ...
                gap: 0,
                padding: '10px 14px',
                borderBottom: i < 2 ? '1px solid var(--fk-border)' : 'none',
                fontSize: 13,
            ...
            >
              ...
              <span style={{ fontFamily: 'var(--fk-font-body)' }}>
                {['alice@co', 'bob@inc', 'carol@ltd'][i]}
              </span>
              <span style={{ fontFamily: 'var(--fk-font-body)' }}>
                {['Pro', 'Enterprise', 'Pro'][i]}
              </span>
              <span style={{ fontFamily: 'var(--fk-font-body)', color: 'var(--fk-success)' }}>
                Active
              </span>
            </div>
          ))}
      ...
      <div style={{ padding: 20 }}>
        ...
        <div className="fk-showcase-mock-grid">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="fk-showcase-mock-card"
            ...
            >
              <div
                style={{
                  ...
                  height: 28,
                  borderRadius: '50%',
                  background: 'var(--fk-accent-subtle)',
                  color: 'var(--fk-accent)',
                  display: 'flex',
                  alignItems: 'center',
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--fk-text)' }}>
                  {
                    ['Acme Corp', 'Blue Inc', 'Cloud Ltd', 'DataMix', 'EdgeFlow', 'GreyHaven'][
                      i - 1
                    ]
                  }
                </div>
...
export function LandingPage({ onSignIn }: { onSignIn: () => void }) {
  ...
  const prevShowcase = useCallback(
    () => setShowcaseIdx((i) => (i - 1 + SHOWCASE_SLIDES.length) % SHOWCASE_SLIDES.length),
    []
  );
        ...
        <div className="fk-nav-inner">
          <div className="fk-nav-left">
            <div className="flex items-center gap-2">
              ...
              <span className="fk-nav-brand">
                ForgeQA{' '}
                <span style={{ color: '#06B6D4', fontSize: '0.85em' }}>License Manager</span>
              </span>
            </div>
          </div>
          <div className="fk-nav-links">
            <button className="fk-nav-link" onClick={scrollTo('features')}>
              Features
            </button>
            <button className="fk-nav-link" onClick={scrollTo('showcase')}>
              Showcase
            </button>
            <button className="fk-nav-link" onClick={scrollTo('testimonials')}>
              Testimonials
            </button>
          </div>
        ...
        <div className="fk-hero-inner">
          <div className="fk-hero-left">
            ...
            <h1 className="fk-hero-title">
              Forge your
              <br />
              <em>license infrastructure.</em>
            </h1>
            <p className="fk-hero-sub">
              Generate keys, manage customers, configure plans, track payments, and verify
              registrations — from one unified dashboard.
            </p>
      ...
      <section className="fk-section" id="features">
        <ScrollReveal>
          <div className="fk-section-header">
            ...
            <p className="fk-section-sub">
              Three steps to get your license infrastructure up and running.
            </p>
          </div>
        </ScrollReveal>
      ...
      <section className="fk-section" style={{ paddingTop: 0 }}>
        <ScrollReveal>
          <div className="fk-section-header">
            ...
            <p className="fk-section-sub">
              A complete toolkit for software licensing operations — no gaps, no workarounds.
            </p>
          </div>
        </ScrollReveal>
        <div className="fk-features-grid">
          {FEATURES.map((feat, i) => {
            ...
            return <FeatureCard key={feat.title} feat={feat} Icon={Icon} index={i} />;
          })}
        </div>
      </section>
          ...
          <div className="fk-showcase-controls">
            ...
            <div className="fk-showcase-dots">
              {SHOWCASE_SLIDES.map((_, i) => (
                <button
                  key={i}
                  className={`fk-showcase-dot-nav${i === showcaseIdx ? ' active' : ''}`}
        ...
        <div className="fk-testimonials-inner">
          <ScrollReveal>
            <div className="fk-section-header">
              ...
              <p className="fk-section-sub">
                Real feedback from teams that rely on ForgeKey daily.
              </p>
            </div>
          </ScrollReveal>
          <div style={{ overflow: 'hidden' }}>
            <div
              className="fk-testimonials-track"
          ...
          <div className="fk-testimonial-controls">
            <button
              className="fk-showcase-btn"
              onClick={prevTestimonials}
              disabled={testimonialsIdx === 0}
              aria-label="Previous"
              style={{ opacity: testimonialsIdx === 0 ? 0.3 : 1 }}
            >
              <ChevronLeft size={16} />
            </button>
            <div className="fk-showcase-dots">
              {Array.from({ length: maxTestimonialIdx + 1 }).map((_, i) => (
                <button
                  key={i}
                  className={`fk-showcase-dot-nav${i === testimonialsIdx ? ' active' : ''}`}
            ...
            <button
              ...
              disabled={testimonialsIdx >= maxTestimonialIdx}
              aria-label="Next"
              style={{ opacity: testimonialsIdx >= maxTestimonialIdx ? 0.3 : 1 }}
            >
              <ChevronRight size={16} />
            </button>
          </div>
        ...
        <div className="fk-footer-inner">
          <div className="fk-footer-top">
            ...
            <div className="fk-footer-links">
              <a href="#" className="fk-footer-link">
                Documentation
              </a>
              <a href="#" className="fk-footer-link">
                API
...
function StatCard({
  ...
  return (
    <div
      className={`fk-stat${visible ? ' fk-visible' : ''}`}
      style={{ transitionDelay: `${delay}ms` }}
    >
