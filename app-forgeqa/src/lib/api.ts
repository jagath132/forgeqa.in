...
export async function regressionGenerate(data: RegressionGenerateRequest) {
  const res = await api.post<{ testCases: TestCase[]; summary: string }>(
    '/api/regression/generate',
    data
  );
  return res.data;
}
...
export async function getLatestBuildArtifact(platform: 'android' | 'web') {
  const res = await api.get<{ artifact: RegressionBuildArtifact | null }>(
    `/api/regression/builds/latest/${platform}`
  );
...
export async function getBuildWebhooks() {
  const res = await api.get<{ webhooks: { platform: string; url: string }[] }>(
    '/api/regression/builds/webhook'
  );
  return res.data.webhooks;
}

export async function startRegistration(data: { name: string; email: string; password: string }) {
  const res = await api.post<{ pendingId: string; email: string }>(
