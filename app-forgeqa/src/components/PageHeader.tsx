...
const pageMeta: Record<string, { title: string; description: string; badge: string; badgeClass: string; gradient: string }> = {
  dashboard: {
    ...
    description: "Monitor test generation activity, review knowledge coverage metrics, and navigate to key workflows.",
    badge: "Dashboard",
  ...
  generator: {
    ...
    description: "Submit product requirements and generate comprehensive test matrices powered by LLM inference.",
...
export function PageHeader() {
  ...
  return (
    <header className="hidden lg:block px-8 py-5 border-b" style={{ background: "var(--bg-glass)", borderColor: "var(--border-default)", backdropFilter: "blur(16px)" }}>
...
export function MobilePageHeader({ pageKey }: { pageKey: string }) {
  ...
  return (
    <div className="lg:hidden px-4 py-4 border-b" style={{ borderColor: "var(--border-default)", background: "var(--bg-glass)", backdropFilter: "blur(16px)" }}>
      ...
      <h1 className="text-lg font-bold" style={{ color: "var(--text-primary)", fontFamily: "var(--font-display)" }}>{meta.title}</h1>
