...
export function RegressionBuildManager() {
  ...
  async function handleSaveWebhook() {
    if (!webhookUrl.trim()) {
      setMessage('Enter a webhook URL.');
      return;
    }
    try {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
            Upload APK or web build, or configure CI/CD webhook.
          </p>
        </div>
      </div>
      ...
      {latestArtifact && (
        ...
        >
          ...
          <span style={{ color: 'var(--text-primary)' }}>
            Latest: <strong>{latestArtifact.fileName}</strong> (v{latestArtifact.version}) —{' '}
            {new Date(latestArtifact.uploadedAt).toLocaleDateString()}
          </span>
        </div>
      )}
        ...
        <form onSubmit={handleUpload} className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            <input
              id="build-file"
              type="file"
              accept={platform === 'android' ? '.apk' : '.zip'}
              className="input-modern flex-1 px-3 py-2 text-xs file:mr-3 file:py-1 file:px-3 file:rounded-lg file:text-xs file:font-semibold file:border-0"
              style={{}}
            />
            <button
              type="submit"
