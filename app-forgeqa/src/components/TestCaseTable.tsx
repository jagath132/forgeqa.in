...
export function TestCaseTable({
  ...
  async function exportPdf() {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text('ForgeQA Test Cases', margin, y);
      y += 24;
      doc.setFontSize(8);
      doc.setFillColor(49, 88, 255);
      ...
      testCaseColumns.forEach((col, i) => {
        const label =
          col.key === 'tcId'
            ...
            : col.key === 'expected'
              ? 'Expected Result'
              : col.key === 'testDescription'
                ? 'Test Description'
                : col.key === 'testSteps'
                  ? 'Test Steps'
                  : col.key.charAt(0).toUpperCase() + col.key.slice(1);
        doc.text(label, x + 4, y + 14, { maxWidth: columnWidths[i] - 8 });
        x += columnWidths[i];
      });
      y += 22;
      ...
      filteredAndSorted.forEach((tc, rowIdx) => {
        const vals = [
          ...
          tc.summary,
          tc.testDescription,
          tc.testSteps.map((s, i) => `${i + 1}. ${s}`).join('\n'),
          tc.expected,
        ];
        ...
        splitCells.forEach((cell, i) => {
          doc.text(cell, x + 4, y + 12, { maxWidth: columnWidths[i] - 8 });
          x += columnWidths[i];
        });
        y += rowH;
      });
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              onChange={(e) => setFilterCategory(e.target.value as TestCaseCategory | 'all')}
            >
              ...
              >
                <th className="px-4 py-3.5 w-10">
                  <input
                    ...
                    checked={
                      selectedRows.size === filteredAndSorted.length && filteredAndSorted.length > 0
                    }
                ...
                {testCaseColumns.map((col) => (
                  <th
                    key={col.key}
                    className={`px-5 py-3.5 text-[11px] font-bold uppercase tracking-wider ${col.sortable ? 'cursor-pointer select-none' : ''}`}
                    style={{ color: 'var(--text-muted)' }}
              ...
              {filteredAndSorted.map((testCase, rowIndex) => {
                ...
                return (
                  <tr
                    ...
                    style={{
                      background:
                        rowIndex % 2 === 0 ? 'var(--bg-secondary)' : 'rgba(247,249,252,0.5)',
                    }}
                    onMouseEnter={(e) => {
                  ...
                  >
                    ...
                    <td className="px-1 py-4">
                      <button
                        onClick={() => toggleRowExpansion(testCase.tcId)}
                        type="button"
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`badge ${categoryStyles[testCase.category] ?? categoryStyles.Positive}`}
                      >
                        {testCase.category}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      {isExpanded ? (
                        <ol
                          className="list-decimal space-y-1 pl-4 text-sm"
                          style={{ color: 'var(--text-secondary)' }}
                        >
                        ...
                        <span className="line-clamp-2" style={{ color: 'var(--text-muted)' }}>
                          {testCase.testSteps.length} step
                          {testCase.testSteps.length !== 1 ? 's' : ''}
                        </span>
                      )}
                    </td>
