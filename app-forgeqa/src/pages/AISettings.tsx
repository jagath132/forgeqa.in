...
export function AISettings() {
  ...
  const activeProviderObj = storeProvider
    ? providerOptions.find((option) => option.id === storeProvider)
    : undefined;
  const selectedProviderObj = selectedProvider
    ? providerOptions.find((option) => option.id === selectedProvider)
    : undefined;
      ...
      {noProviderSaved && (
        ...
        >
          ...
          <div className="text-sm" style={{ color: 'var(--text-primary)' }}>
            <strong>No provider selected yet</strong> — choose one below and save it. Test
      ...
      <Card>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            >
              Choose the LLM provider to power your generation. Keys are encrypted with AES-256-GCM
              and stored securely in the database.
            </p>
          </div>
          <Badge
            variant={hasDbKey ? 'success' : 'warning'}
          ...
          {providerOptions.map((option) => {
            ...
            return (
              ...
              >
                ...
                {isActive ? (
                  <span className="text-xs font-semibold mt-2" style={{ color: 'var(--accent)' }}>
                    Active provider
                  </span>
                ) : (
                  <span
      ...
      <Card>
        ...
        >
          ...
          <span className="badge badge-primary self-start">
            {selectedProviderObj?.label ?? activeProviderObj?.label ?? 'Not selected'}
          </span>
        </div>
