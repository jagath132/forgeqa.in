...
const sections: { id: Section; label: string; icon: string }[] = [
  { id: "profile", label: "Profile & Security", icon: "M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" },
...
export function SettingsPage() {
  ...
  const [billingPlan, setBillingPlan] = useState<{
    tier: string; name: string; monthlyPrice: number; yearlyPrice: number;
    maxUsers: number; maxTestCases: number; aiGenerationsPerDay: number;
    subscriptionStatus: string; subscriptionEndsAt: string | null; features: string[];
  } | null>(null);
  ...
  const [enquiryForm, setEnquiryForm] = useState({ company: "", teamSize: "", contact: "", requirements: "" });
  ...
  async function handleUpgradeSubmit() {
    ...
    try {
      ...
      openConfirm("Upgrade Failed", "Unable to start payment. Please try again later.", () => {}, "Close");
      setUpgradeProcessing(false);
    }
  }
    ...
    switch (activeSection) {
      case "profile": return (
        <div className="max-w-4xl mx-auto">
          {/* Hero */}
          <div className="relative overflow-hidden rounded-2xl mb-8" style={{ background: "linear-gradient(135deg, #2563EB, #38BDF8)", boxShadow: "0 8px 32px rgba(37, 99, 235, 0.2)" }}>
            <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 20% 50%, rgba(255,255,255,0.3) 0%, transparent 60%), radial-gradient(circle at 80% 20%, rgba(255,255,255,0.15) 0%, transparent 50%)" }} />
            <div className="relative px-6 sm:px-8 py-6 sm:py-8 flex flex-col sm:flex-row items-center sm:items-end gap-5">
              <div className="relative shrink-0 cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
                {avatarUrl ? (
                  <img src={avatarUrl} alt="Avatar" className="w-20 h-20 rounded-2xl object-cover ring-4 ring-white/20" />
                ) : (
                  <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.25), rgba(255,255,255,0.08))", backdropFilter: "blur(12px)", border: "2px solid rgba(255,255,255,0.2)" }}>
                    {initials}
                  </div>
                )}
          ...
          <Card className="p-5">
            {/* Header */}
            <div className="flex items-center gap-3 pb-5 mb-5" style={{ borderBottom: "1px solid #F1F5F9" }}>
              <div className="flex h-10 w-10 items-center justify-center rounded-xl" style={{ background: "rgba(37, 99, 235, 0.08)" }}>
                <svg className="h-5 w-5" style={{ color: "#2563EB" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                </svg>
              </div>
              <div>
                <h3 className="text-sm font-bold" style={{ color: "#1E293B" }}>Account Details</h3>
                <p className="text-xs" style={{ color: "#94A3B8" }}>Manage your identity and product access.</p>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex items-center gap-3 sm:w-44 shrink-0">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg shrink-0" style={{ background: "#F1F5F9" }}>
                    <svg className="h-4 w-4" style={{ color: "#64748B" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: "#1E293B" }}>Display Name</p>
                    <p className="text-[11px]" style={{ color: "#94A3B8" }}>How others see you</p>
                  </div>
                </div>
                <div className="flex gap-2 flex-1">
                  ...
                  <button onClick={handleSaveName} disabled={nameSaving}
                    className="btn-primary px-5 py-2.5 text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 shrink-0" type="button"
                  >
                    {nameSaving ? "Saving..." : nameSaved ? <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg> Saved</> : "Save"}
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex items-center gap-3 sm:w-44 shrink-0">
                  ...
                  <div>
                    <p className="text-sm font-semibold" style={{ color: "#1E293B" }}>Email</p>
                    <p className="text-[11px]" style={{ color: "#94A3B8" }}>Your account email</p>
                  </div>
                </div>
                <p className="text-sm flex-1 px-1" style={{ color: "#1E293B" }}>{user.email}</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex items-center gap-3 sm:w-44 shrink-0">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg shrink-0" style={{ background: productKey ? "rgba(16, 185, 129, 0.08)" : "#F1F5F9" }}>
                    <svg className="h-4 w-4" style={{ color: productKey ? "#10B981" : "#94A3B8" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" />
                    </svg>
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold" style={{ color: "#1E293B" }}>Product Key</p>
                      {productKey ? <Badge variant="success">Active</Badge> : <Badge variant="muted">Inactive</Badge>}
                    </div>
                    <p className="text-[11px]" style={{ color: "#94A3B8" }}>License access</p>
                  </div>
                </div>
                {productKey ? (
                  <div className="flex-1 px-1">
                    <p className="text-sm font-mono tracking-wider" style={{ color: "#1E293B" }}>{productKey.key}</p>
                    <p className="text-[11px] mt-0.5" style={{ color: "#94A3B8" }}>Activated {new Date(productKey.activatedAt).toLocaleDateString()}</p>
                  </div>
                ) : (
                  <p className="text-sm flex-1 px-1" style={{ color: "#94A3B8" }}>No product key associated with this account.</p>
                )}
              </div>
            </div>
          </Card>
          ...
          <div className="mt-8">
            <div className="flex items-center gap-3 mb-5">
              <h3 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>Support & Account</h3>
              ...
              <div className="lg:col-span-2">
                {supportSent ? (
                  <Card className="p-12 text-center">
                    ...
                    <p className="mt-3 text-xs" style={{ color: "var(--text-muted)" }}>Form will reset automatically in 10 seconds.</p>
              ...
              <div className="space-y-5">
                <Card className="p-5" style={{ borderColor: "color-mix(in srgb, var(--danger) 30%, transparent)" }}>
                  ...
                  <button onClick={handleLogout} type="button"
                    ...
                    style={{ background: "transparent", color: "var(--danger)", border: "1px solid color-mix(in srgb, var(--danger) 30%, transparent)" }}
                  >
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Manage your subscription and billing information.</p>
            </div>
          ...
          {billingLoading ? (
            ...
            <>
              ...
              <Card className="p-6 mb-6">
                <div className="flex items-start justify-between mb-5">
                  <div className="flex items-center gap-4">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl" style={{ background: billingPlan?.monthlyPrice && billingPlan.monthlyPrice > 0 ? "linear-gradient(135deg, #2563EB, #38BDF8)" : "linear-gradient(135deg, #94A3B8, #CBD5E1)" }}>
                      <svg className="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-xl font-bold" style={{ color: "var(--text-primary)" }}>{billingPlan?.name || "Free"} Plan</p>
                  ...
                  <Badge variant={billingPlan?.subscriptionStatus === "active" ? "success" : "neutral"}>
                    {billingPlan?.subscriptionStatus === "active" ? "Active" : billingPlan?.subscriptionStatus || "Active"}
                  </Badge>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-subtle)" }}>
                  <div>
                    <p className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>Team Members</p>
                    <p className="text-lg font-bold mt-1" style={{ color: "var(--text-primary)" }}>{billingPlan?.maxUsers ?? 1}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>Test Cases</p>
                    <p className="text-lg font-bold mt-1" style={{ color: "var(--text-primary)" }}>{billingPlan?.maxTestCases ? `${billingPlan.maxTestCases.toLocaleString()}` : "Unlimited"}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>AI Generations / Day</p>
                    <p className="text-lg font-bold mt-1" style={{ color: "var(--text-primary)" }}>{billingPlan?.aiGenerationsPerDay ?? 20}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>Status</p>
                    <p className="text-lg font-bold mt-1 capitalize" style={{ color: billingPlan?.subscriptionStatus === "active" ? "var(--accent)" : "var(--danger)" }}>
              ...
              <Card className="p-6 mb-6">
                <h3 className="text-sm font-bold mb-1" style={{ color: "var(--text-primary)" }}>Available Plans</h3>
                <p className="text-xs mb-5" style={{ color: "var(--text-muted)" }}>Choose the plan that fits your team's needs.</p>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    ...
                    return (
                      ...
                      >
                        {plan.popular && !isCurrentPlan && (
                          <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase px-3 py-0.5 rounded-full text-white" style={{ background: "linear-gradient(135deg, #2563EB, #38BDF8)" }}>
                            Most Popular
                          </span>
                        )}
                        <div className="mb-4">
                          ...
                          <div className="mt-2">
                            {isFree ? (
                              <span className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>Free</span>
                            ) : isEnterprise ? (
                              <span className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>Custom</span>
                            ) : (
                              <>
                                <span className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>₹{Math.round((plan.price || 0) / 100).toLocaleString("en-IN")}</span>
                        ...
                        {isCurrentPlan ? (
                          <button disabled className="w-full py-2.5 rounded-lg text-sm font-semibold cursor-default" style={{ background: "var(--bg-secondary)", color: "var(--text-muted)", border: "1px solid var(--border-subtle)" }}>
                            Current Plan
                          </button>
                        ) : plan.id === "enterprise" ? (
                          <button className="w-full py-2.5 rounded-lg text-sm font-semibold cursor-pointer transition-all duration-200" style={{ background: "transparent", color: "var(--accent)", border: "1px solid var(--accent)" }}
                            onClick={() => { setUpgradePlan(plan); setEnquirySent(false); setEnquiryForm({ company: "", teamSize: "", contact: "", requirements: "" }); setShowEnterpriseModal(true); }}>
                            Contact Sales
                          </button>
                        ) : (
                          <button className="w-full py-2.5 rounded-lg text-sm font-semibold text-white cursor-pointer transition-all duration-200" style={{ background: "linear-gradient(135deg, #2563EB, #38BDF8)" }}
                            onClick={() => { setUpgradePlan(plan); setUpgradeBilling("monthly"); setUpgradeProcessing(false); setUpgradeSuccess(false); setShowUpgradeModal(true); }}>
                    ...
                    <div className="col-span-3 text-center py-8 text-sm" style={{ color: "var(--text-muted)" }}>
              ...
              {billingPlan?.monthlyPrice && billingPlan.monthlyPrice > 0 && (
                <div className="grid md:grid-cols-2 gap-6">
                  <Card className="p-5">
                    <div className="flex items-center gap-3 pb-4 mb-4 border-b" style={{ borderColor: "var(--border-subtle)" }}>
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: "var(--accent-amber-soft)" }}>
                        <svg className="h-4 w-4" style={{ color: "var(--accent-amber)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                        </svg>
                      </div>
                      <div>
                        <h3 className="text-sm font-bold" style={{ color: "var(--text-primary)" }}>Billing History</h3>
                        <p className="text-xs" style={{ color: "var(--text-muted)" }}>View your past invoices.</p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between rounded-lg p-4" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-subtle)" }}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{billingPlan.name} — ${billingPlan.monthlyPrice}/mo</p>
                          <p className="text-xs" style={{ color: "var(--text-muted)" }}>Next invoice: {billingPlan.subscriptionEndsAt ? new Date(billingPlan.subscriptionEndsAt).toLocaleDateString() : "N/A"}</p>
                        </div>
                      </div>
                      <button className="btn-secondary px-3 py-1.5 text-xs font-semibold cursor-pointer" type="button"
                        onClick={async () => {
                          try {
                  ...
                  <Card className="p-5">
                    <div className="flex items-center gap-3 pb-4 mb-4 border-b" style={{ borderColor: "var(--border-subtle)" }}>
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: "var(--accent-soft)" }}>
                        <svg className="h-4 w-4" style={{ color: "var(--accent)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                        </svg>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center justify-between rounded-lg p-4" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-subtle)" }}>
                      ...
                      <button className="btn-secondary px-3 py-1.5 text-xs font-semibold cursor-pointer" type="button"
                        onClick={() => openConfirm("Payment Method", "Payment method management will be available soon. Contact jagathwork372@gmail.com for assistance.", () => {}, "Close")}>
                        Update
                      </button>
                    </div>
                  </Card>
    ...
    <div className="animate-fade-in max-w-6xl mx-auto">
      ...
      {createPortal(
        ...
        {showUpgradeModal && upgradePlan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4" style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(12px)" }}
            onClick={closeUpgradeModal}>
            <div className="w-full max-w-[420px] max-h-[90vh] sm:max-h-[85vh] rounded-[20px] sm:rounded-[28px] overflow-hidden relative flex flex-col" style={{ background: "#fff", boxShadow: "0 40px 100px rgba(0,0,0,0.2)" }}
              onClick={(e) => e.stopPropagation()}>
              {/* Close */}
              <button className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-all" style={{ background: "rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.8)" }}
                onClick={closeUpgradeModal}>
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>

              {/* Gradient Header */}
              <div className="relative px-5 sm:px-8 pt-7 sm:pt-10 pb-5 sm:pb-8 text-white shrink-0" style={{ background: "linear-gradient(135deg, #2563EB 0%, #7C3AED 50%, #EC4899 100%)" }}>
                <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(circle at 30% 20%, rgba(255,255,255,0.15) 0%, transparent 50%), radial-gradient(circle at 70% 80%, rgba(0,0,0,0.1) 0%, transparent 40%)" }} />
                <div className="relative">
                  <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4" style={{ background: "rgba(255,255,255,0.15)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.2)" }}>
                    <svg className="w-5 h-5 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
                    </svg>
                  </div>
                  <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] opacity-70 mb-0.5 sm:mb-1">{upgradePlan.name} Plan</p>
                  <h2 className="text-lg sm:text-2xl font-extrabold tracking-tight">Upgrade Your Workspace</h2>
                </div>
              </div>
              ...
              <div className="overflow-y-auto flex-1 min-h-0">
                ...
                <div className="px-5 sm:px-8 pt-5 sm:pt-8 pb-4 sm:pb-6">
                  <div className="flex items-end gap-2">
                    <span className="text-4xl sm:text-[56px] font-black leading-none tracking-tight" style={{ color: "#1E293B" }}>
                      {upgradeBilling === "monthly" ? (
                        <>₹{Math.round((upgradePlan.price || 0) / 100).toLocaleString("en-IN")}</>
                      ) : (
                        <>₹{Math.round(((upgradePlan.price || 0) * 12) / 100).toLocaleString("en-IN")}</>
                      )}
                    </span>
                    <span className="text-sm sm:text-base font-medium pb-0.5 sm:pb-1" style={{ color: "#94A3B8" }}>/{upgradeBilling === "monthly" ? "mo" : "yr"}</span>
                  </div>
                  <p className="text-xs sm:text-sm mt-1.5 sm:mt-2 font-medium" style={{ color: "#64748B" }}>{upgradeBilling === "yearly" ? "Billed annually — save 17%" : "Billed monthly"}</p>
                </div>

                {/* Toggle */}
                <div className="px-5 sm:px-8 mb-4 sm:mb-6">
                  <div className="flex rounded-full p-0.5 sm:p-1" style={{ background: "#F1F5F9" }}>
                    <button className="flex-1 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-full transition-all duration-300"
                      style={{ background: upgradeBilling === "monthly" ? "#1E293B" : "transparent", color: upgradeBilling === "monthly" ? "#fff" : "#64748B", boxShadow: upgradeBilling === "monthly" ? "0 2px 8px rgba(30,41,59,0.2)" : "none" }}
                      onClick={() => setUpgradeBilling("monthly")}>
                      Monthly
                    </button>
                    <button className="flex-1 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-full transition-all duration-300"
                      style={{ background: upgradeBilling === "yearly" ? "#1E293B" : "transparent", color: upgradeBilling === "yearly" ? "#fff" : "#64748B", boxShadow: upgradeBilling === "yearly" ? "0 2px 8px rgba(30,41,59,0.2)" : "none" }}
                ...
                <div className="px-5 sm:px-8 mb-5 sm:mb-8">
                  <div className="grid grid-cols-1 gap-2 sm:gap-3">
                    {upgradePlan.features?.map((f: string, i: number) => (
                      <div key={i} className="flex items-center gap-2.5 sm:gap-3 py-1.5 sm:py-2 px-2.5 sm:px-3 rounded-xl transition-all" style={{ background: "#F8FAFC" }}>
                        <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg, #10B981, #34D399)" }}>
                          <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                          </svg>
                        </div>
                        <span className="text-xs sm:text-sm font-medium" style={{ color: "#334155" }}>{f}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="px-5 sm:px-8 pb-5 sm:pb-8">
                  {upgradeSuccess ? (
                    <div className="text-center py-4">
                      <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center" style={{ background: "linear-gradient(135deg, #10B981, #34D399)" }}>
                        <svg className="w-8 h-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                      </div>
                      <h3 className="text-lg font-bold" style={{ color: "#1E293B" }}>Upgrade Complete!</h3>
                      <p className="text-sm mt-1" style={{ color: "#64748B" }}>Your {upgradePlan.name} plan is now active. A confirmation has been sent to {user.email}.</p>
                      <button className="mt-4 px-6 py-2.5 text-sm font-semibold rounded-xl text-white transition-all" style={{ background: "linear-gradient(135deg, #2563EB, #38BDF8)" }} onClick={closeUpgradeModal}>
                        Done
                      </button>
                    </div>
                  ) : (
                    <button className="w-full py-3 sm:py-4 text-sm font-bold text-white rounded-xl sm:rounded-2xl transition-all duration-300 active:scale-[0.97] flex items-center justify-center gap-2"
                      style={{ background: "linear-gradient(135deg, #2563EB 0%, #7C3AED 100%)", boxShadow: "0 8px 24px rgba(37,99,235,0.35)", opacity: upgradeProcessing ? 0.7 : 1 }}
        ...
        {showEnterpriseModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4" style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(12px)" }}
            ...
            <div className="w-full max-w-[420px] max-h-[90vh] sm:max-h-[85vh] rounded-[20px] sm:rounded-[28px] overflow-hidden relative flex flex-col" style={{ background: "#fff", boxShadow: "0 40px 100px rgba(0,0,0,0.2)" }}
              ...
              {/* Gradient Header */}
              <div className="relative px-5 sm:px-8 pt-7 sm:pt-10 pb-5 sm:pb-8 text-white shrink-0" style={{ background: "linear-gradient(135deg, #0F172A 0%, #1E293B 50%, #334155 100%)" }}>
                <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(circle at 20% 30%, rgba(56,189,248,0.15) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(124,58,237,0.1) 0%, transparent 40%)" }} />
                <div className="relative">
                  <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4" style={{ background: "rgba(255,255,255,0.1)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.1)" }}>
                    <svg className="w-5 h-5 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" />
                    </svg>
                  </div>
                  <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] opacity-60 mb-0.5 sm:mb-1">Enterprise</p>
                  <h2 className="text-lg sm:text-2xl font-extrabold tracking-tight">Contact Sales</h2>
                  <p className="text-xs sm:text-sm mt-1.5 sm:mt-2 opacity-60 leading-relaxed">Get a custom plan for your organization.</p>
                </div>
              </div>
                ...
                <div className="px-5 sm:px-8 py-5 sm:py-6">
                  {enquirySent ? (
                    <div className="text-center py-5 sm:py-8">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full mx-auto mb-3 sm:mb-4 flex items-center justify-center" style={{ background: "linear-gradient(135deg, #10B981, #34D399)" }}>
                        <svg className="w-6 h-6 sm:w-8 sm:h-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                      </div>
                      <h3 className="text-lg sm:text-xl font-bold" style={{ color: "#1E293B" }}>Sent Successfully!</h3>
                      <p className="text-xs sm:text-sm mt-1.5 sm:mt-2 leading-relaxed" style={{ color: "#64748B" }}>We'll get back to you within 24 hours.</p>
                      <button className="w-full mt-5 sm:mt-6 py-2.5 sm:py-3 text-sm font-semibold rounded-xl sm:rounded-2xl transition-all" style={{ background: "#F1F5F9", color: "#334155" }}
                        onClick={() => { setShowEnterpriseModal(false); setEnquirySent(false); setEnquiryForm({ company: "", teamSize: "", contact: "", requirements: "" }); }}>
                        Close
                      </button>
                    </div>
                  ) : (
                    <form onSubmit={async (e) => {
                      ...
                      <div>
                        <label className="block text-[10px] sm:text-xs font-bold mb-1.5 sm:mb-2" style={{ color: "#64748B" }}>Company Name</label>
                        <input type="text" required value={enquiryForm.company}
                          onChange={(e) => setEnquiryForm((p) => ({ ...p, company: e.target.value }))}
                          placeholder="Acme Corp" className="w-full rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3.5 text-sm outline-none transition-all"
                      ...
                      <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                        <div>
                          <label className="block text-[10px] sm:text-xs font-bold mb-1.5 sm:mb-2" style={{ color: "#64748B" }}>Team Size</label>
                          <select value={enquiryForm.teamSize}
                            onChange={(e) => setEnquiryForm((p) => ({ ...p, teamSize: e.target.value }))}
                        ...
                        <div>
                          <label className="block text-[10px] sm:text-xs font-bold mb-1.5 sm:mb-2" style={{ color: "#64748B" }}>Phone</label>
                          <input type="tel" value={enquiryForm.contact}
                            onChange={(e) => setEnquiryForm((p) => ({ ...p, contact: e.target.value }))}
                            placeholder="+1 555-0123" className="w-full rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3.5 text-sm outline-none transition-all"
                            style={{ background: "#F8FAFC", color: "#1E293B", border: "1.5px solid #E2E8F0" }}
                            onFocus={(e) => { e.currentTarget.style.borderColor = "#2563EB"; e.currentTarget.style.boxShadow = "0 0 0 4px rgba(37,99,235,0.08)"; }}
                            onBlur={(e) => { e.currentTarget.style.borderColor = "#E2E8F0"; e.currentTarget.style.boxShadow = "none"; }} />
                        </div>
                      ...
                      <div>
                        <label className="block text-[10px] sm:text-xs font-bold mb-1.5 sm:mb-2" style={{ color: "#64748B" }}>Requirements</label>
                        <textarea required rows={2} value={enquiryForm.requirements}
                          onChange={(e) => setEnquiryForm((p) => ({ ...p, requirements: e.target.value }))}
