  ...
  return (
    <div className="flex min-h-screen" style={{ background: "var(--paper)" }}>
      ...
      <div className="hidden md:flex md:w-[55%] flex-col relative overflow-hidden" style={{ background: "var(--ink)" }}>
        ...
        <div className="absolute top-0 right-0 w-96 h-96 opacity-20" style={{ background: "radial-gradient(circle, rgba(49,88,255,0.3), transparent 70%)" }} />
        <div className="absolute bottom-0 left-0 w-80 h-80 opacity-15" style={{ background: "radial-gradient(circle, rgba(61,217,255,0.25), transparent 70%)" }} />
        ...
        <div className="flex items-center gap-2.5 px-10 pt-10 relative z-10">
          ...
          <span className="text-sm font-bold" style={{ color: "var(--paper)", fontFamily: "var(--font-display)" }}>ForgeQA</span>
        </div>

        {/* Headline */}
        <div className="flex-1 flex flex-col justify-center px-10 relative z-10">
          <h1 className="text-5xl font-extrabold tracking-tight leading-[1.1] mb-3" style={{ fontFamily: "var(--font-display)", color: "var(--paper)" }}>
            Ship with<br />
            <span style={{ background: "linear-gradient(135deg, #3158FF, #3DD9FF, #7C3AED)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>confidence</span>
          </h1>
          <p className="text-base max-w-sm" style={{ color: "rgba(247,249,252,0.5)", fontFamily: "var(--font-sans)" }}>
          ...
          <div className="mt-12" style={{ fontFamily: "var(--font-mono)" }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(18,183,106,0.5)" }}>Test Runner</span>
            </div>
            <div className="rounded-xl p-5" style={{ background: "rgba(0,0,0,0.35)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <ConsoleFeed />
            </div>
          </div>
        </div>
        ...
        <div className="px-10 pb-8 relative z-10">
          <p className="text-xs" style={{ color: "rgba(247,249,252,0.2)", fontFamily: "var(--font-mono)" }}>
      ...
      <div className="w-full md:w-[45%] flex items-center justify-center px-6 py-10 md:py-0" style={{ background: "var(--paper)" }}>
        <div className="w-full max-w-sm">
          ...
          <div className="flex md:hidden items-center gap-2.5 mb-8">
            ...
            <span className="text-sm font-bold" style={{ color: "var(--ink)", fontFamily: "var(--font-display)" }}>ForgeQA</span>
          </div>

          {/* Toggle */}
          <div className="flex p-1 mb-8 rounded-xl" style={{ background: "var(--mist)", border: "1px solid var(--mist)" }}>
          ...
          {error && (
            <div className="mb-5 flex items-start gap-2.5 rounded-xl px-4 py-3 text-sm"
              style={{ background: "rgba(240,68,56,0.06)", color: "var(--danger)", border: "1px solid rgba(240,68,56,0.12)" }}
            >
          ...
          {mode === "forgot" && successMessage ? (
            <div className="space-y-4">
              <div className="flex flex-col items-center py-6 text-center">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: "rgba(18,183,106,0.08)" }}>
                  <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="var(--signal-green)" strokeWidth={1.5}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" autoComplete="off">
              {/* Email */}
              <div>
                <label htmlFor="auth_email" className="block text-xs font-semibold mb-2" style={{ color: "var(--graphite)" }}>Email</label>
                <input id="auth_email" name="auth_email" type="email" required value={email}
                  ...
                  onFocus={(e) => { e.target.style.borderColor = "var(--accent)"; e.target.style.boxShadow = "0 0 0 3px rgba(49,88,255,0.08)"; }}
              ...
              {mode === "login" && (
                <div>
                  <label htmlFor="auth_password" className="block text-xs font-semibold mb-2" style={{ color: "var(--graphite)" }}>Password</label>
                  <div className="relative">
                    <input id="auth_password" name="auth_password" type={showPassword ? "text" : "password"} required value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      placeholder="Enter your password"
                      autoComplete="new-password"
                      className="w-full text-sm outline-none transition-all px-4 py-3 pr-10"
                      style={{
                        color: "var(--ink)",
                        background: "var(--bg-secondary)",
                        border: "1.5px solid var(--mist)",
                    ...
                    <button type="button" onClick={() => setShowPassword(!showPassword)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer" style={{ color: "var(--graphite)", background: "none", border: "none", padding: 4 }}
                    >
                      {showPassword ? (
              ...
              {mode === "login" && require2FA && (
                <div className="animate-fade-in">
                  <div className="p-4 rounded-xl mb-4 text-center" style={{ background: "rgba(18,183,106,0.04)", border: "1px solid rgba(18,183,106,0.12)" }}>
                    <div className="flex items-center justify-center gap-2 mb-2">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="var(--signal-green)" strokeWidth={1.5}>
          ...
          <div className="mt-8 flex justify-center">
            <button onClick={() => navigate("/")} type="button"
              className="text-xs font-semibold transition-all hover:opacity-70 cursor-pointer px-4 py-2 rounded-xl"
