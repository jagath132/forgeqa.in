...
export function createApiMiddleware(env) {
  ...
  return async function apiMiddleware(req, res, next) {
    ...
    try {
      ...
      // 0c. Seed sample knowledge files for testing
      if (url.pathname === '/api/seed-knowledge' && req.method === 'POST') {
        const db = getDb();
        const sampleFiles = [
          { userId: user.id, fileName: "login-api-spec.pdf", fileType: "pdf", sourceType: "upload", status: "ready", chunkCount: 12, uploadDate: "2026-06-28T10:30:00Z", createdAt: "2026-06-28T10:30:00Z" },
          { userId: user.id, fileName: "ui-guidelines.docx", fileType: "docx", sourceType: "upload", status: "ready", chunkCount: 8, uploadDate: "2026-06-29T14:15:00Z", createdAt: "2026-06-29T14:15:00Z" },
          { userId: user.id, fileName: "test-strategy.csv", fileType: "csv", sourceType: "upload", status: "ready", chunkCount: 5, uploadDate: "2026-06-30T09:00:00Z", createdAt: "2026-06-30T09:00:00Z" },
          { userId: user.id, fileName: "product-requirements.txt", fileType: "text", sourceType: "sharepoint", status: "needs_chunking", chunkCount: 0, uploadDate: "2026-07-01T11:45:00Z", createdAt: "2026-07-01T11:45:00Z" },
          { userId: user.id, fileName: "security-audit.pdf", fileType: "pdf", sourceType: "upload", status: "ready", chunkCount: 15, uploadDate: "2026-07-02T16:20:00Z", createdAt: "2026-07-02T16:20:00Z" },
          { userId: user.id, fileName: "release-notes-v2.docx", fileType: "docx", sourceType: "upload", status: "processing", chunkCount: 0, uploadDate: "2026-07-03T08:10:00Z", createdAt: "2026-07-03T08:10:00Z" },
        ];
        const existingCount = await db.collection("knowledge_files").countDocuments({ userId: user.id });
        if (existingCount === 0) {
          await db.collection("knowledge_files").insertMany(sampleFiles);
        }
        sendJson(res, 200, { ok: true, seeded: existingCount === 0 });
        return;
      }
