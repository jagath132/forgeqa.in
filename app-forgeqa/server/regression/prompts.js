export function buildRegressionPrompt({ requirement, existingTestCases, platform }) {
  const existing =
    Array.isArray(existingTestCases) && existingTestCases.length
      ? existingTestCases.map((tc) => `- ${tc.tcId}: ${tc.summary} (${tc.category})`).join('\n')
      : 'No existing test cases provided.';

  const platformGuide = platform === 'mobile' ? 'mobile app (Android APK)' : 'web application';
...
export function buildRegressionScriptPrompt({
  testCases,
  platform,
  framework,
  language,
