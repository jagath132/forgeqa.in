...
function getResend() {
  ...
  if (!resendClient) {
    ...
    if (!key) { resendFailed = true; return null; }
...
export async function sendProductKeyEmailResend(to, productKey, customerName) {
  ...
  if (error) {
    ...
    if (error.statusCode === 401 || error.message?.includes("invalid")) { resendFailed = true; console.warn("Resend API key invalid — disabling Resend for this session"); }
    return false;
  }
