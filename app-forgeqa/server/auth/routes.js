...
async function handleUpgradeSubscription(req, res, url, body, user) {
  ...
  const endsAt = tier === "free" ? null : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
...
async function handleCreateUpgradeCheckout(req, res, url, body, user) {
  ...
  try {
    ...
    const result = await createCheckoutSession({ pendingId, email: user.email, plan: tier, name: user.name || "" });
    sendJson(res, 200, result);
  } catch (err) {
    sendJson(res, 400, { error: err.message });
  }
...
const ROUTE_CONFIG = [
  ...
  { path: "/api/user/billing/upgrade", method: "POST", handler: handleUpgradeSubscription, auth: true },
  { path: "/api/user/billing/checkout", method: "POST", handler: handleCreateUpgradeCheckout, auth: true },
