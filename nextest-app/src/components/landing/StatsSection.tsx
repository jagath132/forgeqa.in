...
function StatCard({ stat, index, visible }: { stat: typeof STATS[0]; index: number; visible: boolean }) {
  ...
  return (
    ...
    >
      <div className="flex-shrink-0 flex items-center justify-center rounded-lg" style={{ width: 40, height: 40, background: "rgba(59,130,246,0.1)" }}>
        <Icon className="w-4 h-4" style={{ color: "var(--neon-blue)" }} />
      </div>
      <div>
        <div className="text-2xl font-bold" style={{ fontFamily: "'Space Grotesk', 'Inter', sans-serif" }}>
          <span className="gradient-text-accent">{count.toLocaleString()}</span>
          <span className="gradient-text-accent">{stat.suffix}</span>
        </div>
        <div className="text-xs mt-0.5" style={{ color: "var(--landing-text-muted)" }}>{stat.label}</div>
      </div>
    </motion.div>
  );
}
