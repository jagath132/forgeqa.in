...
const TIERS = [
  {
    ...
    features: ["500 test generations/mo", "3 AI providers", "CSV & PDF export", "Email support", "Single workspace"],
  ...
  },
  {
    ...
    features: ["5,000 test generations/mo", "All AI providers", "Multi-format export", "Team workspaces", "Priority email support", "Parallel execution"],
    cta: "Start Free Trial",
    highlighted: true,
  },
  {
    ...
    features: ["Unlimited generations", "On-premise deployment", "SSO/SAML", "Dedicated support engineer", "Custom integrations", "SLA guarantee"],
    cta: "Contact Sales",
    highlighted: false,
  },
];
...
function PricingCard({ tier, index, isAuthed, onGetStarted }: PricingSectionProps & { tier: typeof TIERS[0]; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Most Popular badge */}
      {tier.highlighted && (
        <span className="absolute px-4 py-0.5 rounded-full text-[11px] font-bold whitespace-nowrap"
          style={{
            top: "-0.75rem",
            left: "50%",
      ...
      <div className="p-6 lg:p-8 flex flex-col" style={{ position: "relative", zIndex: 10 }}>
        <h3 className="text-lg font-bold" style={{ color: "var(--landing-text)", fontFamily: "'Space Grotesk', sans-serif" }}>{tier.name}</h3>
        <div className="mt-3 flex items-baseline gap-1">
          <span className="text-3xl font-bold gradient-text-accent">{tier.price}</span>
          {tier.period && <span className="text-sm" style={{ color: "var(--landing-text-muted)" }}>{tier.period}</span>}
        </div>
        <p className="mt-2 text-sm" style={{ color: "var(--landing-text-secondary)" }}>{tier.desc}</p>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        >
          ...
          <p className="mt-3 max-w-2xl mx-auto text-base" style={{ color: "var(--landing-text-secondary)" }}>
            Start free, upgrade when you grow. All plans include a 14-day trial with full feature access.
          </p>
        </motion.div>

        <div className="grid md:grid-cols-3 gap-5 lg:gap-8 max-w-5xl mx-auto">
          {TIERS.map((tier, i) => (
            <PricingCard key={tier.name} tier={tier} index={i} isAuthed={isAuthed} onGetStarted={onGetStarted} />
          ))}
        </div>
      </div>
    </section>
