...
const AnvilSvg = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M8 14H16L18 20H6Z" fill="white" fillOpacity="0.12" stroke="white" strokeWidth="1.6" strokeLinejoin="round" />
...
export function ForgeQALogo({ size = "md", showWordmark = true }: LogoProps) {
  ...
  return (
    <div className={`flex items-center ${s.gap}`}>
      ...
      {showWordmark && (
        <div>
          <p className={`${s.text} font-bold gradient-text`} style={{ background: "var(--gradient-rainbow)", backgroundClip: "text", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>ForgeQA</p>
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>Quality at Scale</p>
        </div>
      )}
    </div>
  );
