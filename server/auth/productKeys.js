...
const PRODUCT_KEY_REGEX =
  /^[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{5}-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{5}-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{5}-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{5}-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{5}$/;
...
export async function generateProductKeys(count, metadata = {}) {
  ...
  const existingDocs = await keyColl()
...
export async function listProductKeys(filter = {}) {
  ...
  const docs = await keyColl().find(query).sort({ createdAt: -1 }).toArray();
